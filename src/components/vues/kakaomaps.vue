        ...
        <div style="border-bottom: solid 5px #f56a6a; border-top: solid 5px #f56a6a">
          <div class="row">
            ...
            <!-- 드래그 드랍?? 지우는거 응용할떄 다시 한번 볼듯
              ...
              <draggable v-model="selectlist" group="people" @start="drag = true" @end="drag = false"
                @change="chageevent()">
                <div id="accordion" class="checkout" v-for="element,i in selectlist" :key="element.id">

                  <div class="panel" style=" border-bottom: 1px solid lightslategrey; margin-bottom:0;"
                    draggable="true">
                    <div>
                      <span class="checkout-step-number">{{++i}}</span>
                      <h4 class="checkout-step-title">
                        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne">{{element.name}}</a>
                      </h4>
                    </div>
...
export default {
  ...
  methods: {
    ...
    Overlaydata: function () {
      ...
      (vm.datas = []), (vm.markeris = data), (vm.checkedemotion = []),(nums = 1);
